...
Most changes Copyright 2008-2014 The Python Markdown Project

License: [BSD](https://opensource.org/licenses/bsd-license.php)

Modifications by Błażej Czapp 2021
...
from markdown.extensions.attr_list import get_attrs, AttrListExtension
from markdown.util import parseBoolValue
import re


class NestingFencedCodeExtension(Extension):
    """
    ...
    after preprocessing. Now it's:
    ??? Summary
        ☻wzxhzdk:3♥

    Because FencedCode is a PreProcessor, not a BlockProcessor, it runs first, while
    pymdownx.details runs its BlockProcessor afterwards.
    """
    ...
    def extendMarkdown(self, md):
        """ Add NestingFencedBlockPreprocessor to the Markdown instance. """
        md.registerExtension(self)

        md.preprocessors.register(NestingFencedBlockPreprocessor(md, self.getConfigs()), 'nesting_fenced_code_block', 25)


def detab(block, tab_size):
    lines = block.split('\n')
    for i, line in enumerate(lines):
        if line.startswith(' '*tab_size):
            lines[i] = line[tab_size:]
    return '\n'.join(lines)


class NestingFencedBlockPreprocessor(Preprocessor):
    FENCED_BLOCK_RE = re.compile(
        dedent(r'''
            (?P<indent>[ ]*)(?P<fence>(?:~{3,}|`{3,}))[ ]*           # opening fence
            ((\{(?P<attrs>[^\}\n]*)\})|                              # (optional {attrs} or
            ...
            if m:
                ...
                if self.codehilite_conf and self.codehilite_conf['use_pygments'] and config.get('use_pygments', True):
                    ...
                    highliter = CodeHilite(
                        detab(m.group('code'), len(m.group('indent'))),
                        lang=lang,
...
def makeExtension(**kwargs):  # pragma: no cover
    return NestingFencedCodeExtension(**kwargs)
